...
class QLearningAgent:
    ...
    def act(self, state, epsilon, debug=False, ret_guess=False):
        ...
        return self.q_table[state] if debug else (action, guess, max(self.q_table[state])) if ret_guess else action
...
def drawrect(screen,x,y):
    pygame.draw.rect(screen,RED,[x* ZOOM_SIZE,y * ZOOM_SIZE, ZOOM_SIZE * rect_size_x, ZOOM_SIZE * rect_y])
...
def play_episodes(n_episodes=1_000, max_epsilon=1.0, min_epsilon=0.05, decay_rate=0.0001, gamma=0.99, learn=True, viz=False, human=False, log=False):
    global ball_change_x
    global ball_change_y
    global ball_size_to_sides
    global ball_x
    ...
    for episode in tqdm(range(n_episodes)):
        ...
        while not done:
            ...
            if not human:
                ...
                (action, guess, probability) = agent.act(state=state_to_id[state], epsilon=epsilon, ret_guess=True)
            ...
            if rect_x < ball_x + 1 and ball_x - 1 < rect_x + rect_size_x and ball_y == SPACE_SIZE[1]-1:
                ball_change_y = ball_change_y * -1
                if rect_x + 3 < ball_x + 1 and ball_x - 1 < rect_x + rect_size_x - 3:
                    reward = 6
                #elif rect_x + 1 < ball_x + 1 and ball_x - 1 < rect_x + rect_size_x - 1:
                #    reward = 4
                else:
                    reward = 1
            ...
            if rect_x > SPACE_SIZE[0] - rect_size_x:
                rect_x = SPACE_SIZE[0] - rect_size_x
                reward = -3
                done = True
            ...
            if learn:
                if not guess and probability < 0:
                    agent.learn(state_to_id[old_state], old_action, -3, state_to_id[old_state2], gamma)
...
if learn:
    rewards, epsilon_history = play_episodes(
        n_episodes=3_000, max_epsilon=1.0, min_epsilon=0.00001, decay_rate=0.002, gamma=0.9, learn=True, viz=False, human=False, log=False
    )
