...
# We define the necessary variables: the environment, the number of steps that the agent can take in a given episode and the states that we have encountered so far
tto = TicTacToe()
N_MAX_STEPS_PER_EPISODE = 9
state_to_id = {}


# We encode the given state to a string that we can use for deciding whether if we have encountered this state before or not
...
def play_episodes(n_episodes, max_epsilon=1.0, min_epsilon=0.05, decay_rate=0.0001, gamma=0.9, learn=True, enemy=None):
    ...
    for episode in tqdm(range(n_episodes)):
        ...
        # Old state
        old_state_representation = state_to_str(state)
        old_state_representation2 = state_to_str(state)
        old_action = (0,0)
        ...
        while not done:
            ...
            (action, guess, probability) = agent.act(
                state=state_to_id[state_representation], epsilon=epsilon, ret_guess=True)
            ...
            if learn:
                if not guess and probability < 0:
                    agent.learn(state_to_id[old_state_representation], old_action,
                                -10, state_to_id[old_state_representation2], gamma)
            ...
            old_action = action
            old_state_representation = state_representation
            old_state_representation2 = new_state_representation
            state = new_state
...
if learn:
    ...
    # Summary statistics
    print("Wins:\t", len([r for r in rewards if r > 0.5]))
    print("Ties:\t", len([r for r in rewards if r == 0.5]))
...
rewards, epsilon_history = play_episodes(
    n_episodes=N_EPISODES, max_epsilon=0, min_epsilon=0, decay_rate=0, gamma=0, learn=False, enemy=random_player)

# Summary statistics
print("Wins:\t" + str(len([r for r in rewards if r > 0.5])) + "(" + str(len([r for r in rewards if r > 0.5])/N_EPISODES*100) + "%)")
print("Ties:\t" + str(len([r for r in rewards if r == 0.5])) + "(" + str(len([r for r in rewards if r == 0.5])/N_EPISODES * 100) + "%)")
print("Losses:\t" + str(len([r for r in rewards if r < 0])) + "(" + str(len([r for r in rewards if r < 0])/N_EPISODES * 100) + "%)")
...
while not done:
    ...
    state_representation = state_to_str(state)
    new_state_representation = state_to_str(new_state)
    # print("Agent: \n" + new_state_representation)

    if tto.is_leaf(new_state):
        reward = tto.goodness(new_state, 'X')
        print("Game over" + (lambda :" Tie" if len(new_state['steps']) == 0 else " You lose")())
        print(new_state)
        done = True
    elif new_state_representation == state_representation:
        reward = -1
...
print("Total reward:", total_reward)
#state_representation = state_to_str({'next': 'X', 'result': -1, 'board': {(1, 1): 'X', (3, 3): 'O'}, 'steps': [(2, 1)]})

#print(agent.act(state_to_id[state_representation], 0, True))

#state_representation = state_to_str({'next': 'X', 'result': -1, 'board': {(1, 1): 'X', (3, 3): 'O', (1, 2): 'X', (1, 3): 'O', (2, 3): 'X', (3, 1): 'O'}, 'steps': [(2, 1), (3, 2), (2, 2)]})
